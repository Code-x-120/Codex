...
// 🌟 SECURE FIREBASE URL & PHONE FROM GITHUB SECRETS 🌟
...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (qr && !pairingCodeRequested) {
            ...
            console.log(`📱 Auto-requesting pairing code for: ${phone}`);
            const code = await sock.requestPairingCode(phone);
            const formattedCode = code && code.match(/.{1,4}/g) ? code.match(/.{1,4}/g).join('-') : code;
            console.log('\n==================================================');
            console.log('📱 PAIRING CODE (send this to your WhatsApp):');
            console.log('==================================================\n');
            console.log(`  ${formattedCode}\n`);
            console.log('==================================================');
            console.log('⚠️  Code expires in 60 seconds');
            console.log('==================================================\n');
            console.log('📌 Open WhatsApp > 3 dots > Linked Devices > Link a Device');
            console.log('📌 Tap "Pair with phone number instead" and enter above code\n');
        }

        if (connection === 'open') console.log('✅ CODEX AGENT IS ONLINE!');
        if (connection === 'close') {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (orderStates[sender]?.step === 'WAITING_FOR_ADDRESS') {
            ...
            // Match the exact format of your Codex Agent Admin Panel
            const javaGoatOrder = {
                userId: "whatsapp_" + customerWaNumber,
                userEmail: "Codex1208118@gmail.com",
        ...
        // --- DYNAMIC MENU FEATURE ---
            ...
            let menuMessage = "🍔 *CODEX AGENT MENU* 🍕\n\n";
